/**
 * USDA Press Releases scraper worker — runs in a separate Node process.
 * Site is too slow for direct HTTP. Uses puppeteer.
 */
...
async function init() {
  try {
    ...
    send({ type: 'status', status: 'connecting', message: 'Loading USDA press releases...' });
    await page.goto('https://www.usda.gov/about-usda/news/press-releases', { waitUntil: 'networkidle2', timeout: 45000 });
...
async function poll() {
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      })).filter(i => i.title.length > 20 && i.url !== window.location.href && i.url.includes('/press-releases/20'));
    });
