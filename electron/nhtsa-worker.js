/**
 * NHTSA Press Releases scraper worker — runs in a separate Node process.
 * Site blocks direct HTTP (403), requires puppeteer with stealth.
 */
...
async function init() {
  try {
    ...
    send({ type: 'status', status: 'connecting', message: 'Loading NHTSA press releases...' });
    await page.goto('https://www.nhtsa.gov/press-releases', { waitUntil: 'networkidle2', timeout: 30000 });
...
async function poll() {
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      })).filter(i => i.title.length > 20 && i.url.includes('nhtsa.gov/press-releases/'));
    });
