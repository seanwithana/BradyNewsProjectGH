/**
 * DOJ Press Releases scraper worker — runs in a separate Node process.
 * Site uses bot challenge — requires puppeteer with stealth.
 */
...
async function init() {
  try {
    ...
    send({ type: 'status', status: 'connecting', message: 'Loading DOJ press releases...' });
    await page.goto('https://www.justice.gov/news/press-releases?search_api_fulltext=+&start_date=&end_date=&sort_by=field_date', { waitUntil: 'networkidle2', timeout: 30000 });
...
async function poll() {
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      const links = document.querySelectorAll('a[href*="/opa/pr/"]');
