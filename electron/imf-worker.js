/**
 * IMF World Economic Outlook scraper worker — SPA, requires puppeteer.
 */
...
async function init() {
  try {
    ...
    send({ type: 'status', status: 'connecting', message: 'Loading IMF WEO...' });
    await page.goto('https://www.imf.org/en/publications/weo', { waitUntil: 'networkidle2', timeout: 30000 });
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      return Array.from(links).map(a => ({
        ...
        if (i.title.length < 20 || seen.has(i.url)) return false;
        seen.add(i.url);
        return true;
      });
    });
