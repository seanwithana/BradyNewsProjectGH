/**
 * SEC Press Releases scraper worker — 403 blocked, requires puppeteer.
 */
...
async function init() {
  try {
    ...
    send({ type: 'status', status: 'connecting', message: 'Loading SEC press releases...' });
    await page.goto('https://www.sec.gov/newsroom/press-releases', { waitUntil: 'networkidle2', timeout: 30000 });
  ...
  try {
    ...
    const items = await page.evaluate(() => {
      ...
      return Array.from(links).map(a => {
        ...
        const dateEl = parent ? parent.querySelector('time, .date, [datetime]') : null;
        return {
          url: a.href,
          title: a.textContent.trim(),
          date: dateEl ? dateEl.textContent.trim() : ''
        };
      }).filter(i => i.title.length > 15 && i.url !== window.location.href);
    });

    if (items.length > 0) {
      send({ type: 'status', status: 'connected', message: `Polling OK — ${items.length} items` });
      send({ type: 'items', items: items.map(i => ({ ...i, date: i.date || new Date().toISOString() })) });
    } else {
